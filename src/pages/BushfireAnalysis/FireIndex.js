/**
 * return forecasted bushfire ratings calculated using FFDI model based on climate input
...
export const FFDI = (temp, climate, droughtFactor) => {
  ...
  let humidity = climate
    ? 0.0345 * climate["humidity"]
    : 0.0345 * parseInt(defaultHumidity);
  let temperature = 0.0338 * temp;
  let windspeed = climate
    ? 0.0234 * climate["windspeed"]
    : 0.0234 * parseInt(defaultWindspeed);
